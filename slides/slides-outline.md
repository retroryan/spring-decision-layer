# Slide Outline: The Decision Layer

A proposed outline for a talk on multi-agent Spring AI systems, the Decision
Layer pattern, and this repo's loan-underwriting demo. Written for a
presenter using Marp-style slides (`##` per slide, `---` between slides,
speaker notes in HTML comments).

The outline below runs 19 slides. That is more than one talk slot usually
wants at a comfortable pace. If time is tight, the four "Context Graphs
101" slides (6 through 9) are the easiest to cut or fold into one, since
they are background rather than the demo itself.
...
- **The core claim holds**: this repo really is an intercept-decide-persist
  layer built as a Spring AI advisor, backed by Neo4j. The talk's central
  idea is not aspirational, it is the code.
- **The graph-vs-vector argument is earned**: nothing in this demo is
  embedded. Every "why" answer comes from walking relationships, not from
  comparing text. That is the strongest, most literal part of the talk.
- **The multi-agent ending is a claim, not a demo**: only one agent exists
  (`LoanOfficer`). The repo argues a second agent could read the same graph
  with no new plumbing, but nothing here shows that happening. Say this out
  loud on stage rather than let the audience assume they will see two agents.
- **One live moment is not built yet**: the model cannot grant a brand new
  exception during a run today. Exceptions currently only exist because they
  were seeded. Do not promise the audience "watch it grant an exception live"
  unless that work lands before the talk.

---

## How Well the Demo Covers the Talk

Matched against the abstract, point by point.

- **"Agents share data but not reasoning"**: shown by contrast, not by two
  competing agents. The demo has one agent, so this claim is the setup for
  the ending, not something the audience watches happen twice.
- **"Intercepts every query"**: true. `LoanPolicyAdvisor` is a Spring AI
  `CallAdvisor` and every question passes through it before the model
  answers.
- **"Persists a structured decision trace to Neo4j"**: true. Every run
  writes a `Decision` node with the policy it applied, the numbers it
  measured, who decided it, and what it cited.
- **"Resolves subsequent queries through graph-enriched decision search"**:
  true. Run the same command twice and the second answer is different,
  because the first run's decision is now part of what the graph hands back.
- **"Traverses to the policy, the exception, and every account governed"**:
  true today, using seeded data. The precedent trail already walks all three
  hops. What is missing is a live underwriter granting a brand new exception
  in front of the audience.
- **"Complete multi-agent architecture where agents share reasoning"**: this
  is the biggest gap between the talk description and the repo. The README
  argues it (a portfolio-risk agent could read the same `Decision` nodes),
  and the design genuinely supports it, but no second agent is built. Frame
  the ending as "here is why this generalizes," not "here is the second
  agent."

**Bottom line**: the demo covers the graph-as-decision-layer half of the
talk very well. It covers the multi-agent half as an argument the audience
has to take on the strength of the design, not as something shown.

---

## Does It Use Spring AI Advisors Well

Also plain english.

- **Right seam**: an advisor is the correct place for this. It is the one
  spot every request already passes through, and nothing above it
  (`LoanOfficer`, the prompt) needs to know the graph exists.
- **Correct ordering**: the advisor is placed just before the tool-calling
  advisor, so it runs once per turn instead of once per tool round trip.
  That is a detail a lot of advisor code gets wrong.
- **Parameters passed the right way**: the company id and loan amount come
  in as typed advisor parameters (`.advisors(a -> a.param(...))`), not
  parsed back out of a sentence. That is the pattern Spring AI recommends.
- **Structured output done the native way**: it uses the model's real
  structured-output support (through `AnthropicChatOptions`) instead of
  asking for JSON in the prompt and hoping. That is a best practice, not
  just a working shortcut.
- **Keeps the advisor chain honest**: the advisor sits inside the chat
  memory advisor and rewrites the response before handing it back up the
  chain, so the stored conversation stays plain English instead of a JSON
  blob. That shows real understanding of how advisors compose.
- **One thing to flag on stage, not hide**: this advisor does more than a
  typical advisor. Most advisors log, filter, or add context. This one also
  runs business rules and writes to a database. The repo's own docs defend
  that choice (it is the one place a decision can be made and recorded
  atomically), which is a reasonable call for a demo. A production system
  might split "gather," "decide," and "persist" into separate collaborators
  behind the advisor. Worth one honest sentence in the talk rather than
  presenting it as the only right shape.

**Bottom line**: yes, this is a solid, idiomatic example of the Spring AI
advisor API, and it is a good teaching example precisely because it pushes
the advisor further than most examples do. That extra weight is worth
naming, not smoothing over.
